...
namespace RuntimeGizmos
{
    ...
    public class TransformGizmo : MonoSingleton<TransformGizmo>
    {
        ...
        public KeyCode UndoAction = KeyCode.N;
        public KeyCode RedoAction = KeyCode.M;
        ...
        {
            if (Input.GetKey(ActionKey) || Input.GetMouseButton(1)) return;
        ...
        void GetTarget()
        {
            if (nearAxis == Axis.None && Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
                {
                    ...
                    {
                        ClearAndAddTarget(target);//���� ������Ʈ���� ���õ������� ���� ����ϰ� ���� Ŭ���� ��ü�� Ŭ��
                    }
                }
                else
                {
                    ...
                    {
                        ClearTargets();   //������Ʈ�� �ƴ� ��� Ŭ���� ������ ��ҵ�
                    }
                }
            }
        }

        public void GetTarget(Transform _tr)
        {
            ClearTargets();
            AddTarget(_tr); //Debug.Log("31");
        }
        ...
        public void AddTarget(Transform target, bool addCommand = true)
        {
            if (target != null)
            {
                ...
                //if(addCommand) UndoRedoManager.Insert(new AddTargetCommand(this, target, targetRootsOrdered),eHistoryType.Transform);
        ...
        {
            if (targetRootsOrdered.Count == 0) //����� ������ �ٷ� ����.���ÿ����̴°� �����ϱ�����
                return;
        ...
        void AddTargetHighlightedRenderers(Transform target) //mckim : Ŭ���� ������Ʈ�� ���̶���Ʈ�� ��Ÿ���� �Լ��ΰ�?
        {
            if (target != null) //mckim : ���� Target�� null�� �ƴϸ�
            {
                GetTargetRenderers(target, renderersBuffer); //mckim : Target�� Renderer�� �����´�.

                for (int i = 0; i < renderersBuffer.Count; i++)
                {
                    Renderer render = renderersBuffer[i];

                    if (!highlightedRenderers.Contains(render)) //mckim : ���� highlightedRenderers�� ���ԵǾ����� �ʴٸ�
                    {
                        materialsBuffer.Clear();
                        materialsBuffer.AddRange(render.sharedMaterials);

                        if (!materialsBuffer.Contains(outlineMaterial)) //mckim : ���� materialsBuffer�� outlineMaterial�� ���ԵǾ����� �ʴٸ�
                        {
                            materialsBuffer.Add(outlineMaterial);      //mckim : outlineMaterial�� �߰����ش�.(outline�� Shader��)
        ...
        void GetTargetRenderers(Transform target, List<Renderer> renderers) //mckim : Target�� Renderer�� �������� �Լ�
        {
            renderers.Clear();
            if (target != null)
            {
